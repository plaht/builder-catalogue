...
const Collaborator = ({ initialUserId, user, setId }: CollaboratorProps) => {
  ...
  return (
    <Card
      shadow={false}
      className="max-w-[39rem] px-3 m-2 hover:shadow hover:shadow-grey-800"
    >
      <CardHeader
        floated={false}
        shadow={false}
      ...
      <CardBody className="mb-2 p-0">
        <div className="flex flex-row justify-end pr-3">
          <div className="flex flex-col w-1/3 items-start mb-2">
            <Link
              href={`/user/${initialUserId}/collaborate?collaboratorId=${id}&setId=${setId}`}
            >
              <Button
                size="sm"
                color="green"
          ...
          <div className="flex flex-col w-2/3 items-end">
            <Typography variant="h6" color="blue-gray" className="mt-2">
              {' '}
              {` ${brickCount}`}{' '}
              <FontAwesomeIcon color="green" icon={faCubes} />{' '}
            </Typography>
          </div>
        </div>
      </CardBody>
