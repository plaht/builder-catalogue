...
const addUserToFlattenedCollection = (flattenedCollection: UserCollectionComparison, user: User) => {
    Object.entries(flattenedCollection).forEach(([key, value]) => {
        ...
        for (let idx = 0; idx < value.length; idx++) {
            ...
            if (item.count > foundColor.count) {
                value.splice(idx, 0, { count: foundColor.count, username: user.username, userId: user.id });
                inserted = true;
                break;
            }
        };
        ...
        if (!inserted) { 
            value.push({ count: foundColor.count, username: user.username, userId: user.id });
        }
